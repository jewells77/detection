...
class HGGS_Detector:
    ...
    def __init__(self, version=YOLO_V5, is_cuda=False, onnx_file_path=None, class_list_file_path=None):
        ...
        if onnx_file_path is None:
            if (version == YOLO_V5):
                onnx_file_path = os.path.join(os.path.dirname(__file__), "model/HGGS_model.onnx")
            elif (version == YOLO_V8):
                onnx_file_path = os.path.join(os.path.dirname(__file__), "model/HGGS_model.onnx")

        if class_list_file_path is None:
            class_list_file_path = os.path.join(os.path.dirname(__file__), "model/HGGS_classes.txt")
